...
async function ensureSeed() {
    for (const p of BUILTIN_PROVIDERS) {
        const provider = await prisma_1.default.ai_providers.upsert({
            where: { slug: p.slug },
            update: {
                name: p.name,
                base_url: p.base_url,
        ...
        for (const m of models) {
            await prisma_1.default.ai_models.upsert({
...
router.get("/providers", async (_req, res) => {
    await ensureSeed();
    const providers = await prisma_1.default.ai_providers.findMany({
        orderBy: { id: "asc" },
        include: { ai_models: true },
    });
    const safe = JSON.parse(JSON.stringify(providers, (_, value) => typeof value === "bigint" ? value.toString() : value));
    res.json({ data: safe });
});
router.post("/providers", async (req, res) => {
    ...
    const provider = await prisma_1.default.ai_providers.create({
        data: {
            slug: finalSlug,
            name,
            base_url,
    ...
    const safe = JSON.parse(JSON.stringify(provider, (_, value) => typeof value === "bigint" ? value.toString() : value));
    res.json({ data: safe });
});
router.patch("/providers/:id", async (req, res) => {
    const id = Number(req.params.id);
    const { name, base_url, api_key } = req.body || {};
    const provider = await prisma_1.default.ai_providers.update({
        where: { id },
        data: {
            name: name || undefined,
...
router.post("/models", async (req, res) => {
    ...
    const model = await prisma_1.default.ai_models.create({
        data: {
    ...
    const safe = JSON.parse(JSON.stringify(model, (_, value) => typeof value === "bigint" ? value.toString() : value));
    res.json({ data: safe });
});
router.get("/settings", async (_req, res) => {
    const settings = await prisma_1.default.ai_settings.findUnique({ where: { id: 1 } });
    const safe = JSON.parse(JSON.stringify(settings, (_, value) => typeof value === "bigint" ? value.toString() : value));
    res.json({ data: safe });
});
router.put("/settings", async (req, res) => {
    const { default_provider_id, default_model_id } = req.body || {};
    const settings = await prisma_1.default.ai_settings.upsert({
        where: { id: 1 },
        create: {
            id: 1,
